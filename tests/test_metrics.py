...
def test_plot_results_creates_file(tmp_path):
    ...
    gm = GameMetrics()
    gm.add_agent("AgentF")
    gm.record_result("AgentF", "win")
    gm.record_move("AgentF", 1.0, is_legal=True)
...
def test_plot_performance_radar_runs_without_error():
        ...
        gm.record_move("AgentX", 1.0, is_legal=True)
        gm.add_agent("AgentY")
        gm.record_result("AgentY", "loss")
        gm.record_move("AgentY", 2.0, is_legal=False)
...
def test_plot_performance_radar_mixed_metrics():
        ...
        gm = GameMetrics()
        gm.add_agent("FastAccurate")
        gm.record_result("FastAccurate", "win")
        gm.record_move("FastAccurate", 0.1, is_legal=True)
        gm.add_agent("SlowInaccurate")
        gm.record_result("SlowInaccurate", "loss")
        gm.record_move("SlowInaccurate", 10.0, is_legal=False)
        try:
            gm.plot_performance_radar()
        except Exception as e:
            pytest.fail(f"plot_performance_radar with mixed metrics raised: {e}")
...
def test_plot_move_duration_distribution_no_moves(capsys):
    ...
    gm = GameMetrics()
    gm.add_agent("EmptyAgent")
    gm.plot_move_duration_distribution("EmptyAgent")
    captured = capsys.readouterr()
